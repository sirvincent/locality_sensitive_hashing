# Testing out Locality Sensitive Hashing with banding, shingling & minhashing
#
# The code can be run as follow: python lsh.py <random number generator seed value> <location and filename dataset>
# Assuming one is in the folder containing lsh.py
# example: python lsh.py 42 D:\Users\Vincent\Programming\lsh\user_movie.npy
#
# Created by Vincent Post & Martijn Post 2016

import numpy as np
import sys
from scipy import sparse    # for the sparse matrices
...
def load_data(file_location):
    ...
    """
    # load in data
    data = np.load(file_location)  # first column is user IDs, second column is movie IDs
    ...
    # Create a sparse matrix of (movies by users) fill in 1's if user rated the movie
    col = user_array  # user IDs
    row = movie_array  # movie IDs
    # python counts from 0; 1st user or 1st movie are 0th user and 0th movie in python
    col -= 1
    row -= 1
    ...
    # It is forced to be a boolean because we only have '0' and '1's more information is not needed. Less memory usage
    sparse_data = sparse.csc_matrix((values, (row, col)), shape=(max_movie_id, max_user_id), dtype='b')
...
def minhashing(sparse_data, max_user_id, max_movie_id, user_seed):
    ...
    # allocate memory, force to int32, will only need to maximally count up to max user ID
    signature_matrix = np.zeros((total_permutation, max_user_id)).astype('int32')
    for itPerm in range(total_permutation):
        ...
        # Finding the position of the first '1' in each column and put the position values into the signature matrix
        for itInd in range(maxUserID):
            ar = new_sparse.indices[new_sparse.indptr[itInd]:new_sparse.indptr[itInd + 1]].min()
...
def banding(signature_matrix, total_permutation):
    ...
    # initialize bucket as a list, it is a list because of the different size bucket tuples
    bucket = []
    ...
    for itBands in range(n_bands):
        ...
        # obtain a bucket array in which duplicate vectors in the band are put into the same bucket; the candidate pairs
        ids = np.ravel_multi_index(band.astype(int), band.max(1).astype(int) + 1)
        sidx = ids.argsort()
        sorted_ids = ids[sidx]
        bucket_array = np.array(np.split(sidx, np.nonzero(sorted_ids[1:] > sorted_ids[:-1])[0] + 1))
    ...
    unique_set = set()
    for itUnique in range(len(bucket)):
        # generate a generator expression for the combinations of the pairs in a bucket
        large_user_pair = set(pair for pair in itertools.combinations(bucket[itUnique], 2))

        large_user_pair = large_user_pair.difference(unique_set)
        for userPair in large_user_pair:
            sim = signature_similarity(userPair[0], userPair[1], signature_matrix)
            if sim > 0.5:
                unique_set.add(userPair)
    return unique_set


def creation_txt_file(original_sparse, unique_set):
    """
    ...
    """
    # make from the sparse array a real array, such that we now also put into memory the '0's.
    ...
    #  However the in function is slower with a list, but this is done less often than the iterations
    ...
    #  user2 can > user1
    user_pair_list = []
    # check if the similarity is really > 0.5 with the jaccard similarity and add to the txt file if user1<user2
...
if __name__ == '__main__':
    ...
    signMatrix, totalPermutation = minhashing(sparseData, maxUserID, maxMovieID, userSeed)

    # calculate with Local sensitive hashing with banding the buckets and immediately calculate the unique candidate
    #  pairs set that have a similarity larger than 0.5 by using the signature matrix.
    uniqueSet = banding(signMatrix, totalPermutation)
    ...
    creation_txt_file(originalSparse, uniqueSet)

    end = timer()
    print("Run time (Seconds): " + str(end - start))
